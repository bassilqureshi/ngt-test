...
const SideBar = () => {
  ...
  const sectionNames = {
    ...
    "our-responsibility": "OUR HISTORY",
    "our-board": "OUR BOARD",
    team: "OUR TEAM",
  ...
  return (
    <>
      {visibleItems.length > 0 && (
        <div className="vertical-menu-wrapper  enable lg:flex hidden fixed z-30 lg:top-[70px] 2xl:top-[100px]">
          <div className="relative bg-orange-200 mt-[-25px]">
            <div
              className="absolute top-0 right-0 w-1"
              style={{ height: "100%", transform: "translateX(50%)" }}
            ></div>
            <div
              className="absolute top-0 left-[50px] w-[1px] bg-black"
              style={{
                height: `${progress}%`,
            ...
            {Object.keys(sectionNames).map((section, index) => {
              ...
              return (
                <li
                  key={section}
                  data-target={section}
                  className={`transform transition-transform duration-300 -rotate-90 text-[11px] text-center  cursor-pointer ${
