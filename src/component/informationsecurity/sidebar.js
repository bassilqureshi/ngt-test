...
const SideBar = () => {
  ...
  const updateProgressBar = () => {
    ...
    if (scrollPercent >= 13 && scrollPercent <= 98) {
      if (!isVisible) {
        setIsVisible(true);
        setVisibleItems([]);
  ...
  return (
    <>
      {visibleItems.length > 0 && (
        <div className="vertical-menu-wrapper  enable lg:flex hidden fixed z-30 lg:top-[70px]  2xl:top-[80px]">
          ...
          <ul className="lg:space-y-48 2xl:space-y-32">
            {Object.keys(sectionNames).map((section, index) => {
              ...
              return (
                <li
                  key={section}
                  data-target={section}
                  className={`transform transition-transform duration-300 -rotate-90 text-[11px] ml-[-10px] text-center   cursor-pointer ${isActive ? "text-black" : "text-gray-400"
                ...
                >
                  ...
                  <span
                    className={`container-height block transform ${isActive ? "scale-x-100" : "scale-x-0"
