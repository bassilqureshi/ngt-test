...
const SideBar = () => {
  ...
  const sectionNames = {
    "security-consultancy": "SECURITY CONSULTANCY",
      ...
      {visibleItems.length > 0 && (
        <div className="vertical-menu-wrapper  enable lg:flex hidden fixed z-30 lg:top-[70px]  2xl:top-[140px]">
          <div className="relative  mt-[-57px]">
            ...
            ></div>
            <div
              className="absolute top-0 left-[55px] w-[1px] bg-black"
              style={{
                height: `${progress}%`,
            ...
            {Object.keys(sectionNames).map((section, index) => {
              ...
              return (
                <li
                  key={section}
                  data-target={section}
                  className={`transform transition-transform duration-300 -rotate-90 text-[11px] ml-[-50px] text-center   cursor-pointer ${
