...
const SideBar = () => {
  ...
  const sectionNames = {
    "ngtsol-staff-augmentation": "NGTSOL STAFF AUGMENTATION",
  ...
  const updateProgressBar = () => {
    ...
    if (scrollPercent >= 10 && scrollPercent <= 99) {
      if (!isVisible) {
        setIsVisible(true);
        setVisibleItems([]);
  ...
  return (
    <>
      {visibleItems.length > 0 && (
        <div className="vertical-menu-wrapper  enable lg:flex hidden fixed z-30 lg:top-[140px]  2xl:top-[140px]">
          <div className="relative  mt-[-75px]">
            ...
            ></div>
            <div
              className="absolute top-0 left-[64px] w-[1px] bg-black"
              style={{
                height: `${progress}%`,
